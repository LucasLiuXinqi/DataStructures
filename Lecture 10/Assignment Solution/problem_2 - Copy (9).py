...
class BinarySearchTree(Tree):
    ...
    def delete_range(self, start, end):
        ...
        # Find nodes within the specified range using an iterative in-order traversal
        current = self.first()
        while current is not None:
            if start <= current._element <= end:
                elements_to_delete.append(current._element)
                current = self.after(current)
            elif current._element < start:
                current = self.after(current)
            elif current._element > end:
                break

        # Remove the nodes within the specified range
        for elem in elements_to_delete:
            ...
            while node is not None:
                if node._element == elem:
                    break
                elif node._element > elem:
                    parent = node
                    node = node._left
                elif node._element < elem:
                    parent = node
            ...
            if node._left is None and node._right is None:
                if parent._left == node:
                    ...
                    parent._right = None
                elif parent == node and self._root == elem:
                    return None
            # if only left child
                ...
                if parent.left == node:
                    ...
                    node._left = None
                elif parent == node and self._root == node:
                    node = node._left
                    return node
            # if only right child
            ...
            # if both children are present
                ...
                # get right most child of left subtree
                successor = node._left
                succ_parent = node

                while successor._right is not None:
                    succ_parent = successor
                    successor = successor._right

                if succ_parent == node:
                    node._element = successor._element
                    node._left = successor._left
                    successor = None
                elif succ_parent != node:
                    succ_parent._right = successor._left
                    successor._left = None
                    node._element = successor._element
