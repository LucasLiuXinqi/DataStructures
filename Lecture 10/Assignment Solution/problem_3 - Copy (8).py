...
class BinarySearchTree(Tree):
    ...
    def before(self, node):
        ...
        if node._left is not None:
            ...
            walk = node
            above = walk._parent
            # Keeping looking for the first encountered smaller value from ancester. Either way will work
    ...
    def after(self, node):
        ...
        if node._right is not None:
            ...
            walk = node
            above = walk._parent
            # Keeping looking for the first encountered larger value from ancester. Either way will work
    ...
    def pairs(self, sum1):
        ...
        left = self.first()
        right = self.last()
        while left._element <= right._element:
            if left._element + right._element == sum1:
                return left._element, right._element
            elif left._element + right._element > sum1:
                right = self.before(right)
            else:
                left = self.after(left)

    def pairs_recursive(self, sum1):
        # Please write your code here
        left = self.first()
        right = self.last()

        return self.helper_pairs_recursive(sum1, left, right)
...
class Solution:
    def array_sum(self, arr, sum1):
        ...
        left_p = 0
        right_p = len(arr) - 1
        while arr[left_p] <= arr[right_p]:
            if arr[left_p] + arr[right_p] == sum1:
                return arr[left_p], arr[right_p]
            elif arr[left_p] + arr[right_p] > sum1:
                right_p -= 1
            else:
                left_p += 1
    ...
    def array_sum_recursive(self, arr, sum1):
        ...
        if arr[0] + arr[-1] == sum1:
            ...
            return self.array_sum_recursive(arr[0:-1], sum1)
        else:
...
def main():
    ...
    res = Solution().array_sum(arr, 12)
    print(res)  # should print (3, 9)
