...
class BinarySearchTree(Tree):
    ...
    def diameter(self):
        ...
        def distance(node):
            if node is None:
                return -1, 0 #hight, longest path

            else:
                left_height, left_longest_path = distance(node._left)
                right_height, right_longest_path = distance(node._right)

                node_height = max(left_height, right_height) + 1
                node_path = left_height + right_height + 2

                longest_path = max(node_path, left_longest_path, right_longest_path)

                return node_height, longest_path
