...
class BinarySearchTree(Tree):
    ...
    def delete_range(self, start, end):
        ...
        if self.is_empty():
            return
        tmp = self._subtree_search(self._root, start)
        if end < self.first()._element or start > self.last()._element:
            return
        else:
            while tmp is not None and tmp._element <= end:
                if self._root == tmp and self.num_children(tmp) == 0:
                    self._root = None
                    self._size -= 1
                    return
                else:
                    tmp_next = self.after(tmp)
                    tmp_next = self.after(tmp)
                    self.delete(tmp)
                    tmp = tmp_next
    ...
    def delete(self, node):
        if self.is_leaf(node):
            ...
            if node._left is None:
                ...
                target_node = self._subtree_last_position(node._left)
                if target_node._parent != node:
                    target_node._parent._right = target_node._left
                    if target_node._left is not None:
                        target_node._left._parent = target_node._parent
                    target_node._left = node._left
                    node._left._parent = target_node
                target_node._right = node._right
                node._right._parent = target_node

                if self.is_root(node):
                    self._root = target_node
...
def main():
    ...
    bst.delete_range(30,40)
    #bst.delete(bst.get_node(18))
