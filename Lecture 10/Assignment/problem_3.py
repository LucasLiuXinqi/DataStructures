...
class BinarySearchTree(Tree):
    ...
    def pairs(self, sum1):
        ...
        smallest_node = self._subtree_first_position(self._root)
        biggest_node = self._subtree_last_position(self._root)

        # smallest = smallest_node._element
        # biggest = biggest_node._element

        def find_sum(smallest_node, biggest_node, sum1):
            if smallest_node._element + biggest_node._element == sum1:
                return (smallest_node._element, biggest_node._element)
            elif smallest_node._element + biggest_node._element > sum1:
                biggest_node = self.before(biggest_node)
                return find_sum(smallest_node, biggest_node, sum1)
            elif smallest_node._element + biggest_node._element < sum1:
                smallest_node = self.after(smallest_node)
                return find_sum(smallest_node, biggest_node, sum1)
            elif smallest_node._element == biggest_node._element:
                return None
