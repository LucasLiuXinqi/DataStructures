...
class HeapLinked(PriorityQueueBase, Tree):
    ...
    def _upheap(self, x):
        ...
        while x._parent is not None and x._parent._element._key > x._element._key:
            self._swap(x._parent, x)
            x = x._parent
    ...
    def _downheap(self,y):
        ...
        while y._left is not None:
            smaller_child = y._left
            if y._right is not None and y._right._element._key < y._left._element._key:
                smaller_child = y._right
            if smaller_child._element._key >= y._element._key:
                break
            self._swap(y, smaller_child)
            y = smaller_child
    ...
    def create_next_last(self, x):
        ...
        """
        if x == self._root:
            x._left = self.TreeNode(None, x)
            return x._left
        elif x == x._parent._left:
            x._parent._right = self.TreeNode(None, x._parent)
            return x._parent._right
        else:
            node = self.after(x)
            node._left = self.TreeNode(None, node)
            return node._left


    def after(self, x):
        while x._parent is not None and x._parent._right == x:
            x = x._parent
    ...
    def prev_last(self, x):
        ...
        if x == self._root:
            ...
            return self.before(x)

    def before(self, x):
        while x._parent is not None and x._parent._left == x:
            x = x._parent
        if x._parent is None:
    ...
    def remove_min(self):
        ...
        if self._size == 1:
            ...
            node = self.prev_last(self._last)
            if self._last._parent._left == self._last:
...
class HeapThreadedTree(PriorityQueueBase, Tree):
    ...
    def add_root(self, e):
        ...
        self._root = self.TreeNode(e)
        self._last = self._root
        self._root._prev = None
        self._root._next = None
        self._size += 1
    ...
    def add(self, k,v):
        ...
        if self.is_empty():
            ...
            if self._last == self._root:
                ...
                self._last._parent._right = node
                node._parent = self._last._parent
            else:
                self._last._parent._next._left = node
                node._parent = self._last._parent._next
...
class ProbeHashMap(HashMapBase):
    ...
    def _find_slot(self, j, k):
        ...
        If success was found, success is True and index denotes its location.
        If no success found, success is False and index denotes the NULL slot for insertion.
        """
        success = False
        count = 0
        while self._table[j] is not None and count <= self._scale:
            if self._table[j]._key == k or self._table[j] is None:
                success = True
                break
            else:
    ...
    def _bucket_delitem(self, j, k):
        ...
        if not found:
            ...
            self._table[s] = None
            i = (s + 1) % len(self._table)
            while self._table[i] is not None:
                if self._hash_function(self._table[i]._key) == j:
                    self._table[s] = self._table[i]
                    self._table[i] = None
                    s = i
