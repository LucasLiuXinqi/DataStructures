...
class DoubleLinkedList:
    ...
    def partition_list(self, pivot):
        ...
        self.insertAtFirst(pivot)
        first_loc=self._header
        curr=self._header._next
        piv_loc=self._header._next
        for i in range(len(self)-1):
            if curr==piv_loc:
                curr=curr._next
            if curr._element==pivot:
                self._insert_between(curr._element,piv_loc,piv_loc._next)
            elif curr._element<pivot:
                self._insert_between(curr._element,first_loc,first_loc._next)
                #first_loc=first_loc._next
            elif curr._element>pivot:
                self._insert_between(curr._element,self._trailer._prev,self._trailer)
            if curr._element<pivot:
                first_loc=first_loc._next
            curr_next=curr._next
            self._delete_node(curr)
            curr=curr_next
        self._delete_node(piv_loc)
